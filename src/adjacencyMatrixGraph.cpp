...
void AdjacencyMatrixGraph::addVertex(int id, const std::string& label)
{
...
void AdjacencyMatrixGraph::addEdge(int from, int to, const std::string& label)
{
    ...
    for (size_t i = 0; i < vertices_.size(); ++i)
        for (size_t j = 0; j < vertices_.size(); ++j)
            if (matrix_[i][j].has_value()) 
                edges.push_back({vertices_[i].id, vertices_[j].id, *matrix_[i][j]});
